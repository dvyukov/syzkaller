...
log.Infof(c, "XXXXXXXXXXXXXXXXX: job.Type=%v Flags=%v", job.Type, req.Flags)
...
log.Infof(c, "XXXXXXXXXXXXXXXXX HERE1: job.Type=%v Flags=%v %v/%v/%v/%v/%v/%v", job.Type, req.Flags, BisectResultMerge, BisectResultNoop, job.Flags&BisectResultMerge, job.Flags&BisectResultNoop, (job.Flags&BisectResultMerge) != 0, (job.Flags&BisectResultNoop) != 0)
	if bugReporting == nil || bugReporting.Reported.IsZero() ||
		// Don't report errors for non-user-initiated jobs.
		job.Error != 0 ||
		// Don't report bisections that point to merge
		// or a commit that does not affect kernel binary.
		job.Flags&BisectResultMerge != 0 ||
		job.Flags&BisectResultNoop != 0 {
log.Infof(c, "XXXXXXXXXXXXXXXXX HERE2: job.Type=%v Flags=%v", job.Type, req.Flags)
		job.Reported = true
	} else {
