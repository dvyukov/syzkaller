...
type RPCServer struct {
	TotalBytes atomic.Uint64 // total uncompressed bytes transmitted
...
func RPCCall(addr string, timeScale time.Duration, useCompression bool, method string, args, reply interface{}) error {
	c, err := NewRPCClient(addr, timeScale, useCompression)
	if err != nil {
		return err
	}
	defer c.Close()
