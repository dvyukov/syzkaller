...
func NewRPCServer(addr, name string, receiver interface{}, useCompression bool) (*RPCServer, error) {
	...
	formatMB := func(v int, period time.Duration) string {
		...
		return fmt.Sprintf("%v MB (%v b/sec)", (v+MB/2)/MB, (v+KB/2)/KB/int(period/time.Second))
	}
	serv := &RPCServer{
		ln:             ln,
		s:              s,
		useCompression: useCompression,
		statSent:       stats.Create("rpc sent", "Uncompressed outbound RPC traffic", stats.Graph("RPC traffic"), stats.Rate{}, formatMB),
		statRecv:       stats.Create("rpc recv", "Uncompressed inbound RPC traffic", stats.Graph("RPC traffic"), stats.Rate{}, formatMB),
	}
	return serv, nil
}
