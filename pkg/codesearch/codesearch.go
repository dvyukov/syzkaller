...
var Commands = []Command{
	...
	{"def-source", 2, func(index *Index, args []string) (string, error) {
		ok, file, kind, source, err := index.DefinitionSource(args[0], args[1])
		if err != nil || !ok {
			return notFound, err
		}
...
func (index *Index) DefinitionComment(contextFile, name string) (bool, string, string, string, error) {
	...
	src, err := index.formatSource(def.Comment)
	return true, def.Body.File, def.Kind, src, err
}

func (index *Index) DefinitionSource(contextFile, name string) (bool, string, string, string, error) {
	def := index.findDefinition(contextFile, name)
	if def == nil {
		return false, "", "", "", nil
	}
	src, err := index.formatSource(def.Body)
	return true, def.Body.File, def.Kind, src, err
}
...
func (index *Index) formatSource(lines LineRange) (string, error) {
	...
	for _, dir := range index.srcDirs {
		...
		return formatSourceFile(file, lines.StartLine, lines.EndLine)
	}
