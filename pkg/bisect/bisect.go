...
func (env *env) test() (git.BisectResult, error) {
	...
	tags, err := git.PreviousReleaseTags(cfg.Manager.KernelSrc, current.Hash)
	if err != nil {
		return 0, err
	}
	compiler := "/usr/local/google/home/dvyukov/src/gcc-4.9.4/bin/gcc"
loop:
	for _, tag := range tags {
		switch tag {
		case "v4.12":
			compiler = "/usr/local/google/home/dvyukov/src/gcc-8.1.0/bin/gcc"
			break loop
		case "v4.11":
			compiler = "/usr/local/google/home/dvyukov/src/gcc-7.3.0/bin/gcc"
			break loop
		case "v3.19":
			compiler = "/usr/local/google/home/dvyukov/src/gcc-5.5.0/bin/gcc"
			break loop
		}
	}
	compilerID, err := kernel.CompilerIdentity(compiler)
	if err != nil {
		return 0, err
	}
