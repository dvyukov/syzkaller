...
func (t *FileInfoT) Pack(builder *flatbuffers.Builder) flatbuffers.UOffsetT {
	...
	if t.Data != nil {
		...
		FileInfoStartDataVector(builder, dataLength)
...
func (t *ExecRequestT) Pack(builder *flatbuffers.Builder) flatbuffers.UOffsetT {
	...
	if t.ProgData != nil {
		...
		ExecRequestStartProgDataVector(builder, progDataLength)
		for j := progDataLength - 1; j >= 0; j-- {
			builder.PrependInt8(t.ProgData[j])
		}
		progDataOffset = builder.EndVector(progDataLength)
	}
...
func (rcv *ExecRequest) UnPackTo(t *ExecRequestT) {
	t.Id = rcv.Id()
	progDataLength := rcv.ProgDataLength()
	t.ProgData = make([]int8, progDataLength)
	for j := 0; j < progDataLength; j++ {
		t.ProgData[j] = rcv.ProgData(j)
	}
	t.Flags = rcv.Flags()
...
func (t *CallInfoT) Pack(builder *flatbuffers.Builder) flatbuffers.UOffsetT {
	...
	CallInfoAddErrno(builder, t.Errno)
...
func (rcv *CallInfo) MutateErrno(n int32) bool {
	return rcv._tab.MutateInt32Slot(6, n)
}
...
func CallInfoAddErrno(builder *flatbuffers.Builder, errno int32) {
	builder.PrependInt32Slot(1, errno, 0)
}
...
func (t *ExecResultT) Pack(builder *flatbuffers.Builder) flatbuffers.UOffsetT {
	...
	if t.Output != nil {
		...
		ExecResultStartOutputVector(builder, outputLength)
		for j := outputLength - 1; j >= 0; j-- {
			builder.PrependInt8(t.Output[j])
		}
		outputOffset = builder.EndVector(outputLength)
	}
	errorOffset := builder.CreateString(t.Error)
...
func (rcv *ExecResult) UnPackTo(t *ExecResultT) {
	...
	outputLength := rcv.OutputLength()
	t.Output = make([]int8, outputLength)
	for j := 0; j < outputLength; j++ {
		t.Output[j] = rcv.Output(j)
	}
	t.Error = string(rcv.Error())
