...
func (job *triageJob) deflake(exec func(*queue.Request, ProgFlags) *queue.Result) (stop bool) {
	...
	for run := 1; ; run++ {
		...
		} else if run >= deflakeMaxRuns && prevTotalNewSignal == totalNewSignal {
...
/*
type smashProgJob struct {
	p    *prog.Prog
}

func (job *smashProgJob) run(fuzzer *Fuzzer) {
...
type faultInjectJob struct {
	p    *prog.Prog
	call int
}

func (job *faultInjectJob) run(fuzzer *Fuzzer) {
	fuzzer.Logf(2, "injecting faults into the program %s (call=%d)", job.p, job.call)
	for nth := 1; nth <= 100; nth++ {
		fuzzer.Logf(2, "injecting fault into call %v, step %v", job.call, nth)
		job.p.Calls[job.call].Props.FailNth = nth
		result := fuzzer.execute(fuzzer.smashQueue, &queue.Request{
			Prog: newProg,
			Stat: fuzzer.statExecSmash,
		})
