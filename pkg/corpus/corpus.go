...
type Corpus struct {
	...
	ProgramsList
	//!!! double check which of these need to be exported
	StatProgs  *stats.Val
...
func NewMonitoredCorpus(ctx context.Context, updates chan<- NewItemEvent) *Corpus {
	...
	corpus.StatProgs = stats.Create("corpus", "Number of test programs in the corpus",
		stats.Console, stats.Link("/corpus"), stats.Graph("corpus"), func() int {
			corpus.mu.RLock()
			defer corpus.mu.RUnlock()
			return len(corpus.progs)
		})
	corpus.StatSignal = stats.Create("signal", "Fuzzing signal in the corpus",
		stats.Console, func() int {
			corpus.mu.RLock()
			defer corpus.mu.RUnlock()
			return len(corpus.signal)
		})
	corpus.StatCover = stats.Create("coverage", "Source coverage in the corpus",
		stats.Console, stats.Link("/cover"), stats.Prometheus("syz_corpus_cover"), func() int {
			corpus.mu.RLock()
			defer corpus.mu.RUnlock()
