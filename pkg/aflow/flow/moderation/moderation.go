...
func init() {
	aflow.Register[Inputs, Outputs](
		"moderation",
		"assess if a kernel crash is actionable or not",
		&aflow.Flow{
			Name: "baseline",
			Root: &aflow.Pipeline{
				Actions: []aflow.Action{
					...
					&aflow.LLMAgent{
						...
						Outputs: aflow.LLMOutputs[struct {
							Actionable bool `jsonschema:"If the crash is actionable."`
						}](),
						Temperature: 1,
...
func extractCrashType(ctx *aflow.Context, args extractArgs) (extractResult, error) {
	err := fmt.Errorf("unsupported crash type")
	match := func(pattern string) bool {
		matched := regexp.MustCompile(pattern).MatchString(args.CrashReport)
		if matched {
			err = nil
		}
		return matched
	...
	return extractResult{
		IsUAF:   match("KASAN: .*use-after-free"),
		IsKCSAN: match("KCSAN: data-race"),
	}, err
}
