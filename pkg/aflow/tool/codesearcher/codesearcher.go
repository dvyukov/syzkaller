...
func definitionComment(ctx *aflow.Context, state prepareResult, args defArgs) (defCommentResult, error) {
	ok, file, kind, comment, err := state.Index.DefinitionComment(args.SourceFile, args.Name)
	_ = file
...
func definitionSource(ctx *aflow.Context, state prepareResult, args defArgs) (defSourceResult, error) {
	ok, file, kind, source, err := state.Index.DefinitionSource(args.SourceFile, args.Name)
	_, _ = file, kind
