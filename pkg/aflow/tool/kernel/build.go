...
func buildKernel(ctx *aflow.Context, args buildArgs) (buildResult, error) {
	...
	dir, err := ctx.Cache("build", desc, func(dir string) error {
		...
		makeArgs := build.LinuxMakeArgs(target, targets.DefaultLLVMCompiler, targets.DefaultLLVMLinker, "ccache", dir, runtime.NumCPU())
		if _, err := osutil.RunCmd(time.Hour, args.KernelSrc, "make", append(makeArgs, "bzImage")...); err != nil {
			return err
		}
		genCompileCommands := filepath.Join(args.KernelSrc, "scripts", "clang-tools", "gen_compile_commands.py")
		if _, err := osutil.RunCmd(time.Hour, dir, genCompileCommands); err != nil {
			return err
		}
		return nil
	})
