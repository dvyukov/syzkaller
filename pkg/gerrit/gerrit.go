...
const host = "https://linux-review.git.corp.google.com"

func CreateChange(ctx context.Context, repo, baseCommit, subject, diff string, notifyEmails []string) (
	...
	for _, email := range notifyEmails {
		accounts = append(accounts, map[string]any{"email": email})
	}
	req := map[string]any{
		"project":     project,
		"subject":     subject,
