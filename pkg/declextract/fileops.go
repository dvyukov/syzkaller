...
func (ctx *context) resolveFopsCallbacks() map[*Function]int {
	...
	for _, fops := range ctx.FileOps {
		fops.open = ctx.mustFindFunc(fops.Open, fops.SourceFile)
		fops.read = ctx.mustFindFunc(fops.Read, fops.SourceFile)
		fops.write = ctx.mustFindFunc(fops.Write, fops.SourceFile)
		fops.mmap = ctx.mustFindFunc(fops.Mmap, fops.SourceFile)
		fops.ioctl = ctx.mustFindFunc(fops.Ioctl, fops.SourceFile)
