...
static void checkpoint_iptables(struct ipt_table_desc* tables, int num_tables, int family, int level)
{
	...
	if (fd == -1) {
		...
		// but the fuzzer can manage to remove entry SOCK_STREAM/IPPROTO_TCP using NLBL_MGMT_C_REMOVE,
		// which is unfortunately global (not part of net namespace). In this state creation of such
		// sockets will fail all the time in all processes, but ignoring the error is still probably
		// the best given we allow the fuzzer to invoke NLBL_MGMT_C_REMOVE in the first place.
		case ENOENT:
			return;
		}
