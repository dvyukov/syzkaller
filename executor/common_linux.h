...
static void netlink_data(const void* data, int size)
{
	memcpy(nlmsg.pos, data, size);
	nlmsg.pos += size;
}
...
static void netlink_add_addr(int sock, const char* name, const void* addr, int addrsize)
{
	...
	hdr.ifa_index = if_nametoindex(name);
	netlink_init(RTM_NEWADDR, NLM_F_CREATE | NLM_F_REPLACE, &hdr, sizeof(hdr));
	netlink_attr(IFA_LOCAL, addr, addrsize);
	netlink_attr(IFA_ADDRESS, addr, addrsize);
	int err = netlink_send(sock);
	debug("netlink: add addr %d %s: %s\n", addrsize, name, strerror(err));
}
...
static void initialize_tun(void)
{
	...
	if (!write_file(sysctl, "0")) {
		debug("failed to set sysctl %s\n", sysctl);
	}
	...
	inet_pton(AF_INET, LOCAL_IPV4, &in_addr);
	netlink_add_addr(sock, TUN_IFACE, &in_addr, sizeof(in_addr));
	struct in6_addr in6_addr;
	inet_pton(AF_INET6, LOCAL_IPV6, &in6_addr);
	netlink_add_addr(sock, TUN_IFACE, &in6_addr, sizeof(in6_addr));
...
static void initialize_netdevices(void)
{
	...
	for (i = 0; i < sizeof(devices) / (sizeof(devices[0])); i++) {
		...
		netlink_add_addr(sock, devices[i].name, &in_addr, sizeof(in_addr));
		if (!devices[i].noipv6) {
			sprintf(addr, DEV_IPV6, i + 10);
			struct in6_addr in6_addr;
			inet_pton(AF_INET6, addr, &in6_addr);
			netlink_add_addr(sock, devices[i].name, &in6_addr, sizeof(in6_addr));
		}
		uint64 macaddr = DEV_MAC + ((i + 10ull) << 40);
		netlink_device_change(sock, devices[i].name, true, 0, &macaddr, devices[i].macsize);
	}
	close(sock);
	debug("initialize_netdevices: %llums\n", current_time_ms() - start);
}
...
static void initialize_netdevices_init(void)
{
	...
	for (i = 0; i < sizeof(devtypes) / sizeof(devtypes[0]); i++) {
		...
		uint64 macaddr = 0xbbbbbb + (uint64(i) << (8 * (macsize - 2))) +
				 (procid << (8 * (macsize - 1)));
