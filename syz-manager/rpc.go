...
func startRPCServer(mgr *Manager) (*RPCServer, error) {
	serv := &RPCServer{
		...
		statExecRetries: stats.Create("exec retries", "Number of times a test program was restarted because the first run failed"),
...
func (serv *RPCServer) StartExecuting(a *rpctype.ExecutingRequest, r *int) error {
	...
	runner.mu.Lock()
	defer runner.mu.Unlock()
	req, ok := runner.requests[a.ProgID]
	if !ok {
		return nil
	}
	fmt.Fprintf(runner.injectLog, "executing program %v:\n%s\n", req.req.Prog.Serialize())
	req.executing = true
	req.procID = a.ProcID
	runner.requests[a.ProgID] = req
	return nil
}
...
func (serv *RPCServer) createInstance(name string, injectLog io.Writer) {
	runner := &Runner{
		name:      name,
